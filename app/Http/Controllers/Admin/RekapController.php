...
class RekapController extends Controller
{
    ...
    private function labelFilter(array $f): string
    {
        ...
        if ($f['org']) {
            ...
            if ($j) {
                $bagian[] = $j->unit_label ?: $j->nama;
            }
        }
        if ($f['prof']) {
            $p = Profesi::find($f['prof']);
            if ($p) {
                $bagian[] = 'Profesi '.$p->nama;
            }
        }
    ...
    public function excel(Request $request)
    {
        ...
        if ($mode === 'rekap') {
            ...
            $b = Absensi::select('absensi.*', 'u.nama_lengkap', 'uk.nama AS unit_nama', 'su.nama AS sub_nama',
            ...
            if ($f['unit']) {
                $b->where('u.unit_kerja_id', $f['unit']);
            }
            if ($f['jab']) {
                $b->where('u.jabatan_kategori', $f['jab']);
            }
            if ($f['njab']) {
                $b->where('u.jabatan_id', $f['njab']);
            }
            if ($f['org']) {
                $b->whereIn('u.jabatan_id', app(StrukturService::class)->keturunan($f['org']));
            }
            if ($f['prof']) {
