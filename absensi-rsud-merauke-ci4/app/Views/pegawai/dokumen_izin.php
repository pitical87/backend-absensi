...
<head>
<meta charset="UTF-8">
<title>Dokumen <?= esc($iz['jenis_cuti'] ?: $iz['jenis']) ?> — <?= esc($u['nama_lengkap']) ?></title>
<style>
...
<div class="bar-cetak">
  ...
  <?php if (! empty($bolehTtd) && ! $iz['ttd_digital']): ?>
    <form method="post" action="<?= site_url('izin/tanda-tangan/' . (int) $iz['id']) ?>"
  ...
  <?php endif; ?>
  <span class="info">Nomor surat: <strong><?= esc($iz['nomor_surat'] ?? '—') ?></strong> ·
    Kode verifikasi: <strong><?= esc($iz['kode_verifikasi'] ?? '—') ?></strong></span>
</div>
...
<div class="lembar">
  ...
  <h2 class="judul">Formulir Permohonan <?= esc($iz['jenis_cuti'] ?: $iz['jenis']) ?></h2>
  <p class="nomor">Nomor: <?= esc($iz['nomor_surat'] ?? '—') ?></p>

  <table class="data">
    <tr><td class="label">Nama Pegawai</td><td class="titik">:</td><td><strong><?= esc($u['nama_lengkap']) ?></strong></td></tr>
    <tr><td class="label">NIP</td><td class="titik">:</td><td><?= esc($u['nip'] ?: '—') ?></td></tr>
    <tr><td class="label">Jabatan</td><td class="titik">:</td><td><?= esc(label_jabatan($u)) ?></td></tr>
    <tr><td class="label">Unit Kerja</td><td class="titik">:</td><td><?= esc(unit_organisasi($u)) ?></td></tr>
    <tr><td class="label">Status Kepegawaian</td><td class="titik">:</td><td><?= esc($u['status_pegawai'] ?? 'Non-PNS') ?></td></tr>
    <tr><td class="label">Jenis Permohonan</td><td class="titik">:</td><td><strong><?= esc($iz['jenis_cuti'] ?: $iz['jenis']) ?></strong></td></tr>
    ...
    <tr><td class="label">Alamat Selama Izin/Cuti</td><td class="titik">:</td><td><?= esc($iz['alamat_izin'] ?: '—') ?></td></tr>
    <tr><td class="label">Alasan / Keperluan</td><td class="titik">:</td><td><?= esc($iz['keterangan']) ?></td></tr>
  </table>

  <?php if ($iz['jenis'] === 'Cuti' && $iz['jenis_cuti'] === 'Cuti Tahunan'): ?>
  ...
  <table class="tahap">
    ...
    <tbody>
      <?php foreach ($tahap as $t): ?>
        <?php if ($t['status'] === 'Dilewati') continue; ?>
        <tr>
          <td><?= esc($t['posisi_tahap']) ?></td>
          <td><?= esc($t['oleh_nama'] ?? '—') ?></td>
          <td><?= esc($t['status']) ?></td>
          <td><?= $t['waktu'] ? tgl_id($t['waktu'], false) . ' ' . jam_id($t['waktu']) : '—' ?></td>
          <td><?= esc($t['catatan'] ?? '—') ?></td>
        </tr>
      <?php endforeach; ?>
      <?php if (! array_filter($tahap, fn ($t) => $t['status'] !== 'Dilewati')): ?>
  ...
  <div class="ttd-grid">
    <div class="ttd-blok">Pemohon,<div class="ttd-spasi"></div><strong><?= esc($u['nama_lengkap']) ?></strong></div>
    <?php foreach (array_filter($tahap, fn ($t) => $t['status'] === 'Disetujui') as $t): ?>
    <div class="ttd-blok"><?= esc($t['posisi_tahap']) ?>,<div class="ttd-spasi"></div>
      <strong><?= esc($t['oleh_nama']) ?></strong></div>
    <?php endforeach; ?>
  </div>
</div>
...
<div class="lembar">
  ...
  <h2 class="judul">Surat Keterangan <?= esc($iz['jenis_cuti'] ?: $iz['jenis']) ?></h2>
  <p class="nomor">Nomor: <?= esc($iz['nomor_surat'] ?? '—') ?></p>

  <p style="margin-top:10px">Yang bertanda tangan di bawah ini, Direktur Rumah Sakit Umum Daerah Merauke,
    menerangkan bahwa telah menyetujui permohonan <?= esc(strtolower($iz['jenis_cuti'] ?: $iz['jenis'])) ?>
    bagi pegawai:</p>

  <table class="data" style="margin-top:10px">
    <tr><td class="label">Nama</td><td class="titik">:</td><td><strong><?= esc($u['nama_lengkap']) ?></strong></td></tr>
    <tr><td class="label">NIP</td><td class="titik">:</td><td><?= esc($u['nip'] ?: '—') ?></td></tr>
    <tr><td class="label">Jabatan / Unit Kerja</td><td class="titik">:</td>
        <td><?= esc(label_jabatan($u)) ?> — <?= esc(unit_organisasi($u)) ?></td></tr>
  ...
  <p class="catatan-kecil">Kode verifikasi: <?= esc($iz['kode_verifikasi'] ?? '—') ?> —
    keabsahan surat ini dapat diperiksa melalui <?= site_url('verifikasi') ?></p>
</div>
