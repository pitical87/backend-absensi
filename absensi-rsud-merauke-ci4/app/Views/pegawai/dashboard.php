...
<section class="kartu identitas">
  ...
  <div class="identitas-grid">
    <div class="item"><span>NIP</span><strong><?= esc($u['nip'] ?: '—') ?></strong></div>
    <div class="item"><span>Jabatan</span><strong><?= esc(label_jabatan($u)) ?></strong></div>
    <div class="item"><span>Unit Kerja</span><strong><?= esc(unit_organisasi($u)) ?></strong></div>
    <div class="item"><span>Profesi</span><strong><?= esc($u['profesi_nama'] ?? 'Belum diatur') ?></strong></div>
    <div class="item"><span>Tempat Tugas</span><strong><?= esc(tempat_tugas($u)) ?></strong></div>
    <div class="item"><span>Status Hari Ini</span>
      <strong>
        <?php if ($selesai): ?>Absensi Lengkap
        <?php elseif ($recBuka): ?>Sedang Bertugas
        <?php elseif ($izinHariIni): ?><?= esc($izinHariIni['jenis']) ?>
        <?php else: ?>Belum Absen<?php endif; ?>
      </strong>
    </div>
  </div>
</section>
...
<section class="kartu">
  <div class="kartu-kepala">
    ...
    <span class="badge badge-biru angka"><?= esc(label_shift($u['shift_id'] ? [
...
<section class="kartu">
  ...
  <div class="absen-baris">
    <button type="button" id="btn-datang" class="btn-absen btn-datang" <?= $bolehDatang ? '' : 'disabled' ?>>
      ...
      <span>
        ...
        <small id="ket-datang">
          <?php if ($recTampil && $recTampil['waktu_masuk']): ?>
            Tercatat pukul <?= jam_id($recTampil['waktu_masuk']) ?>
          <?php else: ?>Tekan saat tiba di lokasi RSUD<?php endif; ?>
        </small>
      </span>
    </button>
    ...
    <button type="button" id="btn-pulang" class="btn-absen btn-pulang" <?= $bolehPulang ? '' : 'disabled' ?>>
      ...
      <span>
        ...
        <small id="ket-pulang">
          <?php if ($recTampil && $recTampil['waktu_pulang']): ?>
            Tercatat pukul <?= jam_id($recTampil['waktu_pulang']) ?>
          <?php elseif ($recBuka): ?>Tekan saat mengakhiri tugas hari ini
          <?php else: ?>Lakukan absen datang terlebih dahulu<?php endif; ?>
        </small>
      </span>
    </button>
  </div>
  ...
  <div id="hasil-absen">
    ...
    <?php elseif ($recBuka && $recBuka['tanggal'] !== $hariIni): ?>
    ...
    <?php elseif ($izinHariIni): ?>
      <div class="pesan-hasil pesan-info"><?= ikon('surat', 20) ?>
        <span>Anda tercatat <strong><?= esc($izinHariIni['jenis']) ?></strong>
...
<section class="kartu">
  ...
  <?php if ($rekap['hari_berjalan'] > 0): ?>
    <?php $lebar = 40 + $rekap['hari_dalam_bulan'] * 20; ?>
    ...
    <div class="grafik-bungkus">
      <svg viewBox="0 0 <?= $lebar ?> 160" role="img" aria-label="Grafik jam kerja harian bulan ini">
        <?php foreach ([0, 4, 8, 12] as $j): $y = 130 - $j / 12 * 100; ?>
        ...
        <?php endforeach; ?>
        <?php for ($d = 1; $d <= $rekap['hari_berjalan']; $d++):
            ...
            <?php } elseif (in_array($inf['status'], ['Izin', 'Sakit', 'Cuti', 'Dinas Luar'], true)) { ?>
              <rect x="<?= $x ?>" y="122" width="12" height="8" rx="2" fill="<?= $warna['izin'] ?>">
                <title>Tanggal <?= $d ?>: <?= esc($inf['status']) ?></title>
              </rect>
            <?php } elseif ($inf['status'] === 'Libur') { ?>
  ...
  <div class="tabel-bungkus">
    <table class="tabel">
      ...
      <tbody>
        <?php for ($d = $rekap['hari_berjalan']; $d >= 1; $d--):
            $tgl = sprintf('%04d-%02d-%02d', $tahun, $bulan, $d);
            $inf = $rekap['per_tanggal'][$tgl];
            $rec = $inf['rec'];
        ?>
...
<script>
const ABSEN_CFG = {
  urlAbsen: <?= json_encode(site_url('absen')) ?>,
  urlShift: <?= json_encode(site_url('pilih-shift')) ?>,
