...
<section class="kartu">
  <div class="kartu-kepala">
    ...
    <a class="btn btn-garis btn-kecil" href="<?= site_url('struktur') ?>" target="_blank"
       rel="noopener">Tampilan Pegawai</a>
  </div>
  <div class="tabel-bungkus">
    <?= view('partial/pohon', ['cabang' => $pohon, 'kelola' => true, 'akar' => true]) ?>
  </div>
...
<section class="kartu">
  ...
  <form method="post" action="<?= site_url('admin/struktur/aksi') ?>" class="bilah-alat">
    <?= csrf_field() ?>
    ...
    <select name="kategori" required>
      ...
      <?php foreach ($kategoriJab as $k): ?><option><?= esc($k) ?></option><?php endforeach; ?>
    </select>
    <select name="induk_id">
      <option value="">Induk (atasan langsung)…</option>
      <?php foreach ($semua as $j): ?>
        <option value="<?= (int) $j['id'] ?>"><?= esc($j['nama']) ?></option>
      <?php endforeach; ?>
    </select>
    ...
    <div>
      ...
      <div class="tabel-bungkus">
        <table class="tabel">
          ...
          <tbody>
            <?php foreach (kategori_jabatan_list() as $k): ?>
            <tr><td><?= esc($k) ?></td>
    ...
    <div>
      ...
      <div class="tabel-bungkus">
        <table class="tabel">
          ...
          <tbody>
            <?php foreach ($perUnit as $u): ?>
            <tr><td><?= esc($u['nama']) ?></td><td class="angka"><?= (int) $u['jml'] ?></td></tr>
            <?php endforeach; ?>
            <?php if (! $perUnit): ?>
