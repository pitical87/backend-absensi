<?= $this->extend('layout/admin') ?>
<?= $this->section('isi') ?>

<section class="kartu">
  <div class="kartu-kepala"><h2><?= ikon('gedung') ?> Tambah Unit Kerja</h2></div>
  <form method="post" action="<?= site_url('admin/unit/aksi') ?>" class="bilah-alat">
    <?= csrf_field() ?>
    <input type="hidden" name="aksi" value="tambah_unit">
    <input type="text" name="nama" placeholder="Nama unit kerja baru…" required>
    <label class="teks-kecil" style="display:flex;align-items:center;gap:6px;white-space:nowrap">
      <input type="checkbox" name="punya_sub" value="1" style="width:auto"> Memiliki sub unit
    </label>
    <button type="submit" class="btn btn-primer btn-kecil">+ Tambah</button>
  </form>
</section>

<?php foreach ($unitList as $uk): ?>
<section class="kartu">
  <div class="kartu-kepala">
    <h2><?= ikon('gedung') ?> <?= esc($uk['nama']) ?>
      <span class="badge badge-biru"><?= (int) $uk['jml_pegawai'] ?> pegawai</span></h2>
    <form method="post" action="<?= site_url('admin/unit/aksi') ?>"
          onsubmit="return confirm('Hapus unit <?= esc($uk['nama'], 'js') ?> beserta seluruh sub unitnya?');">
      <?= csrf_field() ?>
      <input type="hidden" name="aksi" value="hapus_unit">
      <input type="hidden" name="id" value="<?= (int) $uk['id'] ?>">
      <button type="submit" class="btn btn-bahaya btn-kecil">Hapus Unit</button>
    </form>
  </div>

  <form method="post" action="<?= site_url('admin/unit/aksi') ?>" class="bilah-alat">
    <?= csrf_field() ?>
    <input type="hidden" name="aksi" value="ubah_unit">
    <input type="hidden" name="id" value="<?= (int) $uk['id'] ?>">
    <input type="text" name="nama" value="<?= esc($uk['nama']) ?>" required>
    <label class="teks-kecil" style="display:flex;align-items:center;gap:6px;white-space:nowrap">
      <input type="checkbox" name="punya_sub" value="1" style="width:auto" <?= $uk['punya_sub'] ? 'checked' : '' ?>>
      Memiliki sub unit
    </label>
    <button type="submit" class="btn btn-navy btn-kecil">Simpan</button>
  </form>

  <?php if ($uk['punya_sub']): ?>
    <h3 style="margin-top:16px">Sub Unit</h3>
    <div class="tabel-bungkus">
      <table class="tabel">
        <thead><tr><th>Nama Sub Unit</th><th>Jumlah Pegawai</th><th style="width:110px">Aksi</th></tr></thead>
        <tbody>
          <?php foreach ($subPerUnit[(int) $uk['id']] ?? [] as $su): ?>
          <tr>
            <td><?= esc($su['nama']) ?></td>
            <td class="angka"><?= (int) $su['jml_pegawai'] ?></td>
            <td>
              <form method="post" action="<?= site_url('admin/unit/aksi') ?>"
                    onsubmit="return confirm('Hapus sub unit <?= esc($su['nama'], 'js') ?>?');">
                <?= csrf_field() ?>
                <input type="hidden" name="aksi" value="hapus_sub">
                <input type="hidden" name="id" value="<?= (int) $su['id'] ?>">
                <button type="submit" class="btn btn-bahaya btn-kecil">Hapus</button>
              </form>
            </td>
          </tr>
          <?php endforeach; ?>
          <?php if (empty($subPerUnit[(int) $uk['id']])): ?>
          <tr><td colspan="3" class="tengah teks-redup">Belum ada sub unit.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>

    <form method="post" action="<?= site_url('admin/unit/aksi') ?>" class="bilah-alat" style="margin-top:12px">
      <?= csrf_field() ?>
      <input type="hidden" name="aksi" value="tambah_sub">
      <input type="hidden" name="unit_kerja_id" value="<?= (int) $uk['id'] ?>">
      <input type="text" name="nama" placeholder="Nama sub unit baru untuk <?= esc($uk['nama']) ?>…" required>
      <button type="submit" class="btn btn-primer btn-kecil">+ Tambah Sub Unit</button>
    </form>
  <?php endif; ?>
</section>
<?php endforeach; ?>

<?= $this->endSection() ?>
