...
<section class="kartu">
  ...
  <form method="get" action="<?= site_url('admin/kehadiran') ?>" class="bilah-alat">
      ...
      <tbody>
        ...
        <tr <?= $r['flag_anomali'] ? 'class="anomali-baris"' : '' ?>>
          <td>
            ...
            <?php if ($r['flag_anomali']): ?>
              <div class="catatan-anomali"><?= ikon('peringatan', 12) ?>
                <?= esc($r['catatan_anomali'] ?? 'Terindikasi anomali GPS') ?></div>
            <?php endif; ?>
          </td>
          <td>
            <div class="foto-mini">
              <?php if ($r['foto_masuk']): ?>
                <a href="<?= site_url('foto/' . (int) $r['id'] . '/datang') ?>" target="_blank"
                   rel="noopener" title="Selfie datang">
                  <img src="<?= site_url('foto/' . (int) $r['id'] . '/datang') ?>" alt="Datang" loading="lazy"></a>
              <?php endif; ?>
              <?php if ($r['foto_pulang']): ?>
                <a href="<?= site_url('foto/' . (int) $r['id'] . '/pulang') ?>" target="_blank"
                   rel="noopener" title="Selfie pulang">
                  <img src="<?= site_url('foto/' . (int) $r['id'] . '/pulang') ?>" alt="Pulang" loading="lazy"></a>
              <?php endif; ?>
              <?php if (! $r['foto_masuk'] && ! $r['foto_pulang']): ?>
          ...
          <td class="angka teks-kecil">
            <?php if ($r['lat_masuk'] !== null): ?>
              <a href="https://www.google.com/maps?q=<?= esc($r['lat_masuk']) ?>,<?= esc($r['lng_masuk']) ?>"
                 target="_blank" rel="noopener">
                <?= esc(number_format((float) $r['lat_masuk'], 5)) ?>, <?= esc(number_format((float) $r['lng_masuk'], 5)) ?>
              </a>
            <?php else: ?>—<?php endif; ?>
          </td>
        </tr>
    ...
    <table class="tabel">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>Absen <?= esc($d['tipe']) ?></td>
          <td class="angka"><?= number_format((float) $d['jarak_meter'], 0, ',', '.') ?> m</td>
          <td class="angka teks-kecil">
            <a href="https://www.google.com/maps?q=<?= esc($d['latitude']) ?>,<?= esc($d['longitude']) ?>"
               target="_blank" rel="noopener">
              <?= esc(number_format((float) $d['latitude'], 5)) ?>, <?= esc(number_format((float) $d['longitude'], 5)) ?></a>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
...
(function () {
  ...
  var rs     = [<?= json_encode($rsLat) ?>, <?= json_encode($rsLng) ?>];
  var radius = <?= json_encode($radius) ?>;
  var titik  = <?= json_encode($titik, JSON_UNESCAPED_UNICODE) ?>;
