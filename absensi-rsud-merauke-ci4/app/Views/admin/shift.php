...
<section class="kartu">
  ...
  <div class="tabel-bungkus">
    <table class="tabel">
      ...
      <tbody>
        ...
        <tr>
          <td><strong>Shift <?= esc($s['kategori']) ?></strong></td>
          ...
          <td>
            <div class="aksi-baris">
              <form method="post" action="<?= site_url('admin/shift/aksi') ?>">
                <?= csrf_field() ?>
  ...
  <form method="post" action="<?= site_url('admin/shift/aksi') ?>" class="bilah-alat">
    <?= csrf_field() ?>
...
<section class="kartu">
  ...
  <form method="get" action="<?= site_url('admin/shift') ?>" class="bilah-alat">
    <input type="text" name="q" placeholder="Cari nama pegawai…" value="<?= esc($q) ?>">
    <select name="unit">
  ...
  <div class="tabel-bungkus">
    <table class="tabel">
      ...
      <tbody>
        ...
        <tr>
          <td><strong><?= esc($p['nama_lengkap']) ?></strong></td>
          <td><?= esc($p['unit_nama'] ?? '—') ?><?= $p['sub_nama'] ? ' — ' . esc($p['sub_nama']) : '' ?></td>
          <td><?= esc($p['profesi_nama'] ?? '—') ?></td>
          <td>
            <form method="post" action="<?= site_url('admin/shift/aksi') ?>" class="bilah-alat" style="margin:0">
              <?= csrf_field() ?>
